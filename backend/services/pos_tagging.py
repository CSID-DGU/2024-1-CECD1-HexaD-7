...
class PororoMecabPos(PororoSimpleBase):
    ...
    def _postprocess(self, morpheme):
        surface = morpheme.surface
        pos_tag = morpheme.feature.pos
        if '+' in pos_tag:  # 복합 태그 분리
            sub_tags = pos_tag.split('+')
            sub_surfaces = morpheme.feature.expression.split('+') if morpheme.feature.expression else [surface] * len(sub_tags)
            return [(sub_surf.split('/')[0], sub_tag) for sub_surf, sub_tag in zip(sub_surfaces, sub_tags)]
        else:
            return [(surface, pos_tag)]
    ...
    #     results = []
    #     sent_ptr = 0  # 문장 내 위치 추적
    #     analyzed = self._model.parse(sent)

    #     for morpheme in analyzed:
    #         sub_results = self._postprocess(morpheme)
    #         # 형태소가 시작되는 지점까지 공백 확인 및 추가
    #         while sent_ptr < morpheme.span.start:
    #             if sent[sent_ptr] == ' ':
    #                 results.append((' ', 'SPACE'))
    #             sent_ptr += 1
    #         # 형태소 결과 추가
    #         results.extend(sub_results)
    #         sent_ptr = morpheme.span.end  # 형태소 끝 위치로 포인터 업데이트

    #     # 문장의 남은 부분에 대한 공백 처리
    #     while sent_ptr < len(sent):
    #         if sent[sent_ptr] == ' ':
    #             results.append((' ', 'SPACE'))
    #         sent_ptr += 1
